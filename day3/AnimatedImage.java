 ...
 */
public class AnimatedImage extends DrawingGUI { // extends DrawingGUI to get functionality
	...
	public void handleTimer() {
		for (int b = 0; b < numToMove; b++) {
			// on each timer tick, randomly select 1,000 WanderingPixels, then have those step()
			// Pick a random blob and ask it to move.
			blobs.get((int)(Math.random()*blobs.size())).step();
		}
		// Now update the drawing
		repaint(); //repaint() screen when done
	}
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				// load background image, pass image to constructor
			}
		});
	}
}
