...
public class BlobsGUI2 extends DrawingGUI {
	...
	//blobType keeps track of what kind of blob to add next
	...
	public BlobsGUI2() {
		super("Animated Blobs", width, height); // uses DrawingGUI bc it "extends" and is a subclass of DrawingGUI
		...
		blobs = new ArrayList<Blob>(); // instantiated in the constructor
	...
	public void handleMousePress(int x, int y) {
		// Check if hit a blob
		// for each loop, keeps checking each item in the list
		for (Blob/*variable type*/ blob/*variable(can be any name)*/ : blobs /*ArrayList name*/) {
			if (blob.contains(x, y)) { // checks to see if mouse is inside any blob
				System.out.println("back off!"); // if it is, back off
				return;
			}
		}

		// if for reach loop does not trigger a return, a new blob is created and is added to ArrayList
	...
	public void handleTimer() {
		...
		for (Blob blob : blobs) { // tells all the blobs to step
			blob.step();
		}
		// Now update the GUI.
		repaint(); // repaint
