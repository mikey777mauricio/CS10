...
public class RegionFinderEC {
	...
	public ArrayList<Point> findSecondLargest() {
		ArrayList<Point> largest = findLargestRegion();
		ArrayList<Point> second = findLargestRegion();
		if (regions.size() > 1) {
			second = regions.get(0); // set region to first index of regions
			for (ArrayList<Point> region : regions) {
				if (region.size() < largest.size() && region.size() > second.size()) {
					second = region;
				}
				return second;
			}
