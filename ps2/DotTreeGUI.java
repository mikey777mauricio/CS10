...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			if(tree == null){
				tree = new PointQuadtree<Dot>(new Dot(x,y), 0,0,width,height); // start with A
			}
			else {
	...
	private void test2() {
		found = null;
		tree = new PointQuadtree<Dot>(new Dot(400,300), 0,0,width,height); // start with A
		tree.insert(new Dot(200,450)); // B
		tree.insert(new Dot(150,550)); // C
		tree.insert(new Dot(450,200)); // D
		tree.insert(new Dot(240,250)); // E
		tree.insert(new Dot(550,475)); // F

		System.out.println(tree.allPoints());
		System.out.println(tree.size());

		int bad = 0;
		bad += testFind(190,450,10,6,3,1); // 1 hit
		bad += testFind(400,300,10,6,5,1); // 1 hit
		bad += testFind(600,450,10,5,2,0); 	// no hits
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		// TODO: YOUR CODE HERE
		// draw blob
		g.fillOval((int)(tree.getPoint().getX())-dotRadius, (int)(tree.getPoint().getY())-dotRadius, dotRadius, dotRadius);
		//g.drawString(String.valueOf(level), (int)tree.getPoint().getX(), (int)tree.getPoint().getY());
		// draw lines
		...
		// TODO: YOUR CODE HERE
		for(int i = 0; i <= 4; i++) {
			if (tree.hasChild(i)) drawTree(g, tree.getChild(i), level + 1); // check all quadrants for children
		}
