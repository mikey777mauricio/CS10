 ...
 *
 * uses 3 images
 * scenery: static image, this is where we want you to appear to be located
 * backgrouund: static snapshot of the camera's view without you in it
 * image: live image as it comes from the camera
 *
 */
public class WebcamBg extends Webcam {
	...
	//threshold: if the color is close enough, then color did not change
	...
	public void handleMousePress(int x, int y) {
		if (image != null) {
			//saves current image as the background
	 ...
	 * Updates image so that pixels that look like background are replaced with scenery.
	 ...
	 loops over every pixel, picks up each image and color
	 and the background image
	 calculates the distance in color
	 if it is less than the threshold, that means the color did no change
	 Draw scenery
	 updates image at x and y to have scenery
	 otherwise the image at x and y will have camera image
	 */
	@Override
