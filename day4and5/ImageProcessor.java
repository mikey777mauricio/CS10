...
public class ImageProcessor {
	 ...
	 * Will often need to constrain values to fall within a range
	 * Idea is to create a helper method and call it where needed, rather than duplicating range checking code in each function
	 * Static --> all objects share same variable
	 * Ex. Many objects and wanted to keep number of objects --> static amount of objects, each object will
	 * have that variable
	 * Static Method can be called without instantiating object (Math.random()) no neeed to instantiate
	 * Math object
	 *
	 */
	...
	public void average(int radius) {
		...
		/* to get every x and y location on the screen
			3 component to a c-style for loop, init, condition, increment
			declaring y in the for loop will allow it to live through the entier for loop
			state in the for loop while the condition is true
			eventually, y will become equal to the height and go through all the x's


		Spins around each pixel, picks up neighboring pixel, sum the RGB, average it out
					 */
	...
	public void dim() {
		...
		for (int y = 0; y < image.getHeight(); y++) {
			for (int x = 0; x < image.getWidth(); x++) {
				// together a "nested loop", Loop over every y value and loop over every x value
				//will cover all x and y locations
				//x will increment through first
			    // Get current color; scale each channel; put new color
			    Color color = new Color(image.getRGB(x, y));
			    int red = color.getRed() * 3/4;
			    int green = color.getGreen() * 3/4; //scale color by 75%
			    int blue = color.getBlue() * 3/4;
			    Color newColor = new Color(red, green, blue);
			    image.setRGB(x, y, newColor.getRGB()); // write changes back to image
				// image is an instance variable holding the image we are manipulating
			}
		}
	}
	 ...
	 * Loop over every row
	 * Calculate the "target" or y2, gethieght() - 1 - y (row 0 goes to row height -1)
	 *
	 *
	 *
	 * To solve without second image: temp = RGB(x,y)
	 * tamp = a
	 * setRGB(x,y, getRGB(x,y2))
	 * a = b
	 * set(RGB(x,y2,temp)
	 * b = temp
	 */
	public void flip() {
		...
		// Make the current image be this new image.
		image = result;
	}	// if not, local variable would have gone outta scope and image would not hae changed
	...
	public void drawSquare(int cx, int cy, int r, Color color) {
		...
		// loop from r rows avove to r rows below mouse
		// Draw scaure around mouse location given color
		//Pixel at x,y: x-1 x x+1
		//
		//			y-1
		//			y
		//			y+1
		// Dont go outside of image
