...
public class BinaryTree<E> {
	...
	// copy to depth method PD 3
	public BinaryTree<E> copyToDepth(int d) {
		BinaryTree<E> newTree = new BinaryTree<E>(data);
		copyToDepthHelper(newTree, d);
		return newTree;
	}


	public void copyToDepthHelper(BinaryTree<E> tree, int d){
		if(d >= 0){
			if(hasLeft()){
				tree.left = left;
				tree.getLeft().copyToDepthHelper(tree, d--);}
			if(hasRight()){
				tree.right = right;
				getRight().copyToDepthHelper(tree, d--);
			}
		}
...
//		System.out.println(t2.copyToDepth(2));
