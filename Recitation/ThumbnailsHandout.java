...
public class ThumbnailsHandout extends DrawingGUI {
	...
	private ArrayList<BufferedImage> thumbs = new ArrayList<>();		// thumbnail images to display
	...
	private static BufferedImage thumbnailify(BufferedImage image) {
		...
		for(int y = 0; y < result.getHeight(); y++)
		{
			...
			{
				result.setRGB(x, y, image.getRGB(x*image.getWidth()/result.getWidth(), y* image.getHeight()/result.getHeight()));
			}
		}
